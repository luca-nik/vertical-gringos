        ...
        <div className="grid lg:grid-cols-2 gap-8 md:gap-12 lg:gap-16 items-center">
          ...
          <p className="text-base md:text-lg text-slate-100 max-w-2xl mx-auto font-light leading-relaxed">
            ...
            <div className="pt-6 md:pt-8">
              <p className="text-sm md:text-base text-slate-400 leading-relaxed mb-6 md:mb-8">
                Un weekend che unisce sci alpino e musica elettronica in alta quota. 
                Dalle piste innevate ai beat che scaldano il cuore, 
                un&apos;esperienza di puro flow state nella bellezza dell&apos;inverno alpino.
              </p>

              <motion.button
                whileHover={{ scale: 1.05, y: -2 }}
                transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }}
                className="px-8 md:px-10 py-3 md:py-4 bg-slate-900 text-ice-white rounded-full hover:bg-slate-800 text-xs md:text-sm font-medium tracking-widest uppercase"
              >
          ...
          >
            <div className="relative h-80 md:h-96 lg:h-[500px] rounded-3xl overflow-hidden shadow-2xl">
              ...
              {/* Winter Overlay */}
              <div className="absolute inset-0 bg-gradient-to-t from-blue-900/20 via-transparent to-blue-100/10" />
              
              {/* Floating Badge */}
              <motion.div
                animate={{ y: [0, -8, 0] }}
                transition={{ duration: 3, repeat: Infinity, ease: "easeInOut" }}
                className="absolute top-4 md:top-6 right-4 md:right-6 bg-ice-white/90 backdrop-blur-sm px-3 md:px-4 py-1.5 md:py-2 rounded-full"
              >
                <span className="text-slate-900 text-xs md:text-sm font-semibold tracking-wide text-slate-400">WINTER 2025</span>
              </motion.div>
            </div>
